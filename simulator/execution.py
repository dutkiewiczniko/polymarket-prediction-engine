    ...
    *,
    ...
    if action == "buy_up":
        if sell_opposite_first and portfolio.down_tokens > 0:
            events.extend(_sell_down(portfolio, timestamp, down_price, reason="exit down before buy_up"))
        events.extend(_buy_up(portfolio, timestamp, up_price, usd_amount, reason))
        return events

    if action == "buy_down":
        if sell_opposite_first and portfolio.up_tokens > 0:
            events.extend(_sell_up(portfolio, timestamp, up_price, reason="exit up before buy_down"))
        events.extend(_buy_down(portfolio, timestamp, down_price, usd_amount, reason))
        return events

    if action == "sell_up":
        events.extend(_sell_up(portfolio, timestamp, up_price, reason))
        return events

    if action == "sell_down":
        events.extend(_sell_down(portfolio, timestamp, down_price, reason))
        return events
...
def _buy_up(portfolio: Portfolio, timestamp: str, price: float, usd_amount: float, reason: str) -> list[TradeEvent]:
    if price <= 0 or portfolio.cash < usd_amount:
        return []
...
def _sell_up(portfolio: Portfolio, timestamp: str, price: float, reason: str) -> list[TradeEvent]:
...
def _sell_down(portfolio: Portfolio, timestamp: str, price: float, reason: str) -> list[TradeEvent]:
